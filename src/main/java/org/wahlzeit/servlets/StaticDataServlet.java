	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if ("image".equals(type)) {
				...
				if (image != null) {
					...
					log.warning(LogBuilder.createSystemMessage()
							.addMessage("image not found").toString());
				...
				log.warning(LogBuilder
						.createSystemMessage()
						.addMessage(
								"unimplemented static resource type has been requested")
						.toString());
			...
			log.severe(LogBuilder.createSystemMessage()
					.addException("Problem when loading image", e).toString());
		}
	}
	...
	private Image getImage(String photoId, int size) {
		...
		if (image == null) {
			...
			try {
				...
				log.warning(LogBuilder.createSystemMessage()
						.addException("Problem when reading image.", e)
						.toString());
			}
